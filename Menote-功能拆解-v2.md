...
**技术可行性说明（供项目架构设计，本文不展开）**：

- 账户增加一对非对称密钥：公钥明文存储，锁定时也能使用；私钥用主钥 MK 包裹后存储。
- 锁定时移入：浏览器为该条目生成新的数据密钥 DEK，用 DEK 加密正文、附件和旧版本；再用公钥加密这把 DEK 后存储。
- 锁定时拿不到空间密钥 DEK_s，因此这类条目的标题先用条目自己的 DEK 加密；下次解锁时，客户端用 MK 解开私钥、再解开 DEK，把标题改为用 DEK_s 加密（DEK 也可在此时改为由 MK 包裹，与其他条目一致）。
- 密钥对何时生成（启用隐私锁时生成；已启用的账户何时补生成）由架构文档确定。
- 若架构评估认为此方案不可行，用户的备选方案是只做前端锁定：锁定时移入的条目以明文存储，只在界面上按锁定处理（与 Memo 隐私浏览同类的界面保护）。这种方式在服务端、本机缓存和备份中都是明文，保护强度明显较弱；采用时确认框需如实说明，是否在下次解锁时再补做真正的加密届时再定。
...
- 中断后在下次解锁时继续；锁定时发起的移入任务（M08-09）只用到公钥，中断后在下次打开应用时即可继续，不必解锁。每篇文章任何时刻要么完整明文、要么完整密文。
- 移动或标记变更在全部转换完成后才提交。
...
stateDiagram-v2
    [*] --> 明文
    明文 --> 转换中: 开启单篇加密，或在解锁状态下移入加密空间
    明文 --> 锁定移入中: 锁定时移入加密空间（仅限空间根目录）
    转换中 --> 加密: 全部完成后提交
    转换中 --> 转换中: 中断后下次解锁继续
    锁定移入中 --> 锁定移入中: 中断后下次打开应用继续（无需解锁）
    锁定移入中 --> 待整理标题: 全部完成后提交（标题暂用条目自己的 DEK 加密）
    待整理标题 --> 加密: 下次解锁时标题改用 DEK_s 加密
    加密 --> 解密中: 取消加密或移出加密空间（需解锁）
    解密中 --> 明文: 全部完成后提交
    加密 --> 加密: 在加密空间内移动（不重新加密）
```

说明：转换中（包括锁定移入中）的条目对 MCP 已不可见；每篇文章任一时刻要么完整明文、要么完整密文。“待整理标题”的条目正文、附件、旧版本都已是完整密文，只是标题还没有改用空间密钥，锁定时对用户不可见，解锁后自动完成转换（M08-09）。
