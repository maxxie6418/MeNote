...
| 版本 | v2.1（文件名保持 `Menote-功能拆解-v2.md` 不变，避免打断既有引用） |
| 日期 | 2026-09-26 |
| 状态 | 第三稿。已在 v2 基础上按原型定稿同步一轮；已确认的问题见 10.0；其余待确认问题确认前，相关功能点仍按“建议”写法暂定 |
...
| 通用 | 启动视图；时区；界面偏好 = **主题**（浅色 / 深色 / 跟随系统，默认浅色，2026-09-26 定） | M02-04 |
| 编辑器 | 默认编辑模式 | M04-03 |
...
关于子页面与原型的关系：原型设置页目前平铺了 8 张卡片（通用 / 账户与安全 / 隐私锁 / Memo 隐私浏览 / MCP 令牌 / 备份目标 / 我的分享 / 版本与回收站），其中把「Memo 浏览需要隐私密码」单独成卡。上表的「编辑器 / 数据管理 / 实例管理」三页原型暂未展示，属**原型省略**、不构成冲突；「Memo 隐私浏览」在上表里归入隐私锁子页面，界面上可独立成卡。
