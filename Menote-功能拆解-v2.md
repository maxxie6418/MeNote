...
| 版本 | v2.2（文件名保持 `Menote-功能拆解-v2.md` 不变，避免打断既有引用） |
| 日期 | 2026-09-26 |
| 状态 | 第四稿。v2.1 之后补上设置页的两栏分页结构（M18-01），原型与框架页同步；已确认的问题见 10.0；其余待确认问题确认前，相关功能点仍按“建议”写法暂定 |
...
- 页面顶部另有一条全宽顶栏，从左到右为 **6 块**（按原型 `.topbar`）：品牌、面包屑、搜索框（M09-01）、同步状态（M02-06）、全局隐私状态胶囊（M02-05）、**账户与设置**。
  - 账户入口是一个圆形头像（原型 `#topAccount`，字母取自用户名首字），紧邻隐私锁胶囊右侧；只显示头像、不显示用户名，`title` 里给出提示，点击直接进入设置、不弹菜单。
...
- 「实例管理」只对 owner 显示，导航项上带 `owner` 标记；它属**实例级设置**（`site_settings`），与个人设置（`user_settings`）分开存放（§7.5 两级归属）。
...
| 子页面 | 设置项 | 功能点 |
|---|---|---|
| 通用（默认落地页） | 启动视图；时区；界面偏好 = **主题**（浅色 / 深色 / 跟随系统，默认浅色，2026-09-26 定） | M02-04 |
...
|---|---|---|---|---|
| 新增① | 顶栏块数与账户入口 | 顶栏为 **6 块**，末尾增加「账户与设置」圆形头像，点击直接进设置、不弹菜单；功能栏底部不再有设置入口 | 与 §7.4 功能栏表格“底部｜设置入口”、§7.5“入口位于功能栏底部”不同，经用户确认；设计文档待同步 | M02-01、M02-02 |
...
### 需求文档待同步的用户决定（v2 新增，v2.1 补充 10–16 条）
...
10. **顶栏**（§7.4 顶部、§7.5）：顶栏为 6 块，末尾增加「账户与设置」圆形头像入口（只显示头像，点击直接进设置）；删除 §7.4 功能栏表格的“底部｜设置入口”与 §7.5 的“入口位于功能栏底部”。
