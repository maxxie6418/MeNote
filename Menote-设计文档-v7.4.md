...
| 区域 | 内容 |
|---|---|
| 新建按钮 | 打开新建菜单：笔记 / 表格 / 文件夹 |
| 快速录入框 | 多行文本输入框，下方为模式选择：**Memo（默认）/ 待办 / 笔记**，发布后按所选模式创建内容（见下） |
| 导航 | 最近编辑、收藏、笔记本（即文件夹树）、标签、隐私空间（即加密空间，锁定状态与图标见 6.9） |
| 底部 | 设置入口（7.5） |
...
| 模式 | 行为 |
|---|---|
| Memo（默认） | 等同 8.7 的快速输入框：Ctrl+Enter 发布，进时间轴；写入永远免密（8.9） |
| 待办 | 等同 Memo + 清单标记（9.2），提供日期与优先级的快捷选择【细则后续定】 |
| 笔记 | 以输入框内容新建一篇笔记并打开编辑器（首行是否作为标题【推荐：是，细则后续定】） |

表格不在快速录入框的模式中：表格需要先定义列结构（10.2），走笔记本中的新建入口。
...
| 通用 | 时区（默认 Asia/Shanghai）；界面偏好【细则后续定】 | 8.2 |
...
- 快速录入框（功能栏顶部，见 7.4，模式选择默认 Memo）：**Ctrl+Enter（macOS 为 Cmd+Enter）发布**，Enter 换行；移动端使用发送按钮。
- 支持 `#标签` 语法。
...
- 统一在快速录入框（7.4）：选择"待办"模式，或在 Memo 模式下打开"清单"开关，这条 Memo 即成为任务条目。
...
7. **界面布局细则**：7.4 为整体设想；各视图的具体形态、快速录入框三模式的细节（如笔记首行是否作标题、待办的日期与优先级快捷选择）、滑出详情侧栏的适用场景、窄屏与移动端（PWA）适配，在界面闭环设计时再定。
...
| 界面布局 | 左右结构：功能栏（新建按钮 + 快速录入框 + 导航）+ 主操作区；快速录入框模式为 Memo（默认）/ 待办 / 笔记；导航为最近编辑、收藏、笔记本、标签、隐私空间；笔记本用列表 + 正文双栏（借鉴 Inkstone）；表格与笔记混排、仅编辑样式不同（类 Notion 数据库表格）；部分场景用滑出详情侧栏 |
