...
| `docs/scratch.md` | **草稿**：临时讨论与零散想法的追加区，随手写、不设格式门槛；定稿内容搬往 todo/ / modules/ / wiki，其余定期清理，不作为实现依据 | 单文件 |
...
3. **归档**：`docs/archive/` 只允许整份移入，移入后在头部加一行「已归档，结论见 adr-NNN / wiki 某文档」；归档件不再修改，原位置不留副本。现有功能拆解 v1 在结构迁移时移入。`docs/todo/` 的计划做完整份移入；`docs/scratch.md` 只清内容不归档整份。
4. **进入门槛（防增殖）**：能写进本文既有章节的一句话决定，不单开 ADR；一次就能定的事不建专项文档；`guides/` 只收"会被重复执行的操作"，一次性的事直接做。文档数量超需要时宁可删并，不维护僵尸文件。

> **迁移时机注**：当前三份主线文档还在仓库根目录、`wiki/` 与 `docs/` 尚未创建——在初始化代码骨架时按本节一次性搬到位。搬动本身属于结构变更，执行前先经用户确认。
