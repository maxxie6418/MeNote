...
#### 2.3.4 docs/：文档组织与生命周期【已定·用户确认 2026-09-26】

目录职责一览：

| 位置 | 放什么 | 命名 |
|---|---|---|
| 仓库根目录 | 需求文档、功能拆解（当前有效的两份主线文档，放根目录醒目处） | 既有文件名，升版本不改名 |
| `docs/Menote-项目架构-v1.md` | 架构文档（本文，唯一主线） | 不变 |
| `docs/components.md` | **组件规划**：前端组件清单——组件名、所属 feature、职责、props 约定、复用关系、与原型的对应关系；单文件过长后按 `components/<功能>.md` 拆 | — |
| `docs/proposals/` | **方案讨论草稿**：未定稿的探索、多方案对比、评审记录 | `proposal-<主题>-<YYYYMMDD>.md` |
| `docs/adr/` | **架构决定记录**：只记"有备选项且否决了其他选项"的决定，一条一页（背景 / 备选 / 结论 / 影响） | `adr-NNN-<决定名>.md` |
| `docs/guides/` | **操作指南**：本地开发上手、一键部署步骤（15.5 的操作展开）、测试跑法、迁移演练 | `<主题>.md` |
| `docs/archive/` | **归档**：被取代的草稿与旧版文档，只读 | 移入时保留原文件名 |

生命周期（所有文档适用）：

1. **草稿期**：讨论中的方案进 `proposals/`，一份主题一份文件，不散落在聊天记录里；文档头部注明状态（讨论中 / 已定稿 / 已否决）。
2. **定稿**：结论写回对应主线文档（架构决定进本文、需求变更进需求文档）+ 有备选项的补一条 ADR；随后草稿**整份移入** `archive/`，头部加一行「已归档，结论见 adr-NNN」。同主题不允许出现胜负未分的并行多份草稿——赢了进 ADR，输了整份归档。
3. **归档规则**：`archive/` 内文件不再修改，只允许整份移入；被新版取代的旧文档同样移入（原位置不留副本、不留"最新版"软链）。现有功能拆解 v1 在仓库结构调整时移入。
4. **进入门槛（防增殖）**：能写进本文既有章节的一句话决定，不单开 ADR；一次就能定的事不建 proposal；`guides/` 只收"会被重复执行的操作"，一次性的事直接做。文档数量超需要时宁可删并，不维护僵尸文件。
