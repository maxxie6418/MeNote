...
> **状态：✅ 完成（v0.2.20）**。逐项复核：
> - **BroadcastChannel（跨标签页）**：✅ **已完成**。广播层与引擎接线（事件只放"哪条变了/游标到哪"，无 `BroadcastChannel` 时降级为空实现；收到事件**只重读本地库**不跑同步，并用 in-flight 标记合并连发事件）；同条目并发编辑的**事前提示**也已落地（正文区琥珀色提示条 + 「按最新内容重新载入」，自己保存不误报；静默覆盖本身另有保存时的 `rev_conflict` 兜底）。
...
> - **冲突通知栏「对比两者 / 保留某一份」**：✅ 已完成（v0.2.20）。本地 conflicts 表记「副本 → 原条目」关联（不靠标题）；正文区提示条 + 查看副本 + 保留我的版本 + 保留服务端版本。**副本一律保留**，删除随 M4 的回收站（现在删不可逆）。
