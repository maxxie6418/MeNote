...
|---|---|
| 文档版本 | v1 |
| 文档状态 | 评审中（M1 收口后再开工；开工前需完成 DESIGN.md 视觉章定稿与 Q 项拍板） |
...
### 1.3 M1 遗留清单（M2 开工前或开工初期清掉，别丢）

M1 已收口（`docs/archive/Menote-M1-实施计划-v1.md`），但有几项当时判定"可移入 M2"或依赖真机，集中记在这里：

| # | 遗留项 | 来源 | 说明 |
|---|---|---|---|
| 1 | **真机两设备验证** | M1-11 验收 | 同机两个浏览器 profile 已验过（含断网补传）；还缺**手机 + 桌面**两台真机走"注册 → 建笔记 → 互见 → 断网编辑 → 联网补传"。顺带量一次手机上 PBKDF2 600k 的耗时（风险表 #2）——**桌面机代理值已测：中位数 95 ms、最慢 100 ms**（Node 22 WebCrypto，与浏览器同参），离 2 秒红线很远，预计手机端无需降到 310k |
...
### 1.4 待拍板 Q 项（建议值来自功能拆解第十章）

### 1.2 待拍板 Q 项（建议值来自功能拆解第十章）
| Q | 事项 | 建议 |
|---|---|---|
| Q19 | Memo 如何编辑 | 时间轴中原位展开为输入框，`Ctrl+Enter` 保存、`Esc` 取消 |
| Q21 | 三个设置项归属 | "全部离线缓存"→通用；Memo 快照周期→备份；全量 ZIP 导出→数据管理 |
| Q23 | 去掉清单标记时是否删除 `task` 字段 | 删除 |
| Q26 | 快捷菜单放不放"新建" | 第一版不放 |
| Q9 | Memo 能否置顶 | 可以；置顶显示在时间轴最上方，待办视图不受影响 |
| Q17 | 标签能否重命名/合并/删除 | 第一版不做 |
| Q12 | 删除文件夹的三件套 | 确认框写清影响数量；连同内容进回收站并保留原路径；恢复时原文件夹不在则回根目录 |
| Q24 | 标题与正文第一行是否联动 | 按钮新建不预填一级标题；标题仍为「未命名笔记」且正文首行是一级标题时自动取用；用户手动改过后不再联动 |
| Q4 | "仅本次查看"档位下 Memo 死路 | 该档位下在 Memo 区域点解锁只解除 Memo 门禁，离开视图即重新锁定（M3 才生效，M2 留策略位） |
| Q6 | 门禁锁定时刚写的 Memo 怎么显示 | 提示"已发布（隐私浏览中，解锁后可见）"，时间轴仍占位 |
| Q7 | 首页锁定时的 Memo 部分 | 「今日待办」「最近动态」中来自 Memo 的部分显示"已锁定"占位；统计卡片始终计入 |
