...
> **状态：✅ 完成（v0.2.11）**。落地：`features/tasks/model.ts`（按状态分组、组内"有截止在前 → 优先级 → 最近更新"排序、状态/优先级/截止三档纯本地筛选、状态计数、卡片标题取正文首行）；`actions.ts`（改状态/截止/优先级/去掉清单标记——**同时**改 md 的 front matter 与条目派生列并入队元数据补丁）；`TaskPanel`（列表 / 看板切换）、`TaskListView`、`TaskKanban`（三列固定顺序）、`TaskFilterBar`、`TaskCard`（列表与看板共用）。录入框待办档接入**真实控件**（原生日期输入 + 优先级三段），新建清单默认状态"待办"，发布后保持待办模式。任务字段字面量的约束以 **0002 迁移的 6 个触发器**落地（见《数据模型与迁移设计》v1.3：补 CHECK 需重建热表，触发器等价且非破坏）。
>
> **口径说明**：看板改状态用卡片上的**文字按钮**（开始 / 完成 / 重开）——需求写的是"点卡片改状态**或**拖到另一列"，按钮那条无依赖且无障碍友好；拖拽留到移动端适配时再补（不把拖拽当唯一入口）。**"带图片时设为清单置灰"随 M4**（M2 没有图片）。
